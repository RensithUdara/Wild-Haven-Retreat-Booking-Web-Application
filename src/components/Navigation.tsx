...
import { Menu, X, Tent, UserRound } from "lucide-react";
    ...
    <div className="container mx-auto px-6 lg:px-12 py-5">
      <div className="flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-10">
          ...
          <Link to={user ? "/account" : "/auth"} className={`flex items-center gap-1.5 text-[11px] uppercase tracking-wider font-normal smooth-hover hover:opacity-60 ${isDark || !isScrolled ? "text-white" : "text-foreground"}`}>
            <UserRound className="h-3.5 w-3.5" />
            {user ? "Bookings" : "Sign In"}
          </Link>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && <motion.div initial={{
          ...
          <Link to={user ? "/account" : "/auth"} className={`block py-3 text-[11px] uppercase tracking-wider font-normal smooth-hover hover:opacity-60 ${isDark || !isScrolled ? "text-white" : "text-foreground"}`} onClick={() => setIsMobileMenuOpen(false)}>
            {user ? "Bookings" : "Sign In"}
          </Link>
